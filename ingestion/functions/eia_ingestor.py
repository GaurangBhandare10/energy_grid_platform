...
GRID_REGIONS = {
    ...
    "CAL":  "California ISO",              # ← fixed from CALI
...
if __name__ == "__main__":
    ...
    # TEMPORARY — show per region breakdown
    print("\nPer region breakdown:")
    for region, result in summary['results'].items():
        print(f"  {region:<6} -> status={result.get('status'):<10} reason={result.get('reason','')}")